...
public abstract class ScaleImageUtils {
     ...
     */
     ...
     public static InputStream resizeToInputStream(int width, int height, String suffix,Image targetImage) throws ImageFormatException, IOException {
         width = Math.max(width, 1);
         height = Math.max(height, 1);
         BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
         image.createGraphics().drawImage(targetImage, 0, 0, width, height, null);
         
         ByteArrayOutputStream os = new ByteArrayOutputStream();  
         ImageIO.write(image, suffix.substring(1), os);  
     ...
     * @throws IOException
     */
    public static InputStream resize(float scale,String suffix, InputStream targetFile) throws ImageFormatException, IOException {
        image = ImageIO.read(targetFile);
        int[] size = getSize(scale, image);
        return resizeToInputStream(size[0], size[1], suffix,image);
    }
